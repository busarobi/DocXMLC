...
\vspace{\sectionBefore}
\subsection{Inconsistency of HSM}
\label{sec:online_PLTs}
\vspace{\sectionAfter}
...
$$
\prob(\by \given \bx) = \prod_{j=1}^m \prob(y_j \given \bx)\,,
$$
the optimal solution in terms of precision@k does not change. To show this let us observe the following.
Let $y_i$ and $y_j$ be so that $\prob(y_i = 1 \given \bx) \ge \prob(y_j = 1 \given \bx) $. Then in the summation over of $\by$s in~(\ref{eqn:mapping}), we are interested in four different subsets of $\calY$: 
$$
...
$$
y_t \frac{\prob(\by \given \bx)}{\sum_{t'=1}^m y_{t'}} = y_t \frac{\prod_{t'=1}^m \prob(y_{t'} \given \bx)}{\sum_{t'=1}^m y_{t'}}
$$ 
