...
To validate the theoretical results we compared the performance of PLTs and HSM on synthetic datasets of three types: multi-label data with independent labels, multi-label data with conditionally dependent labels and multi-class data. 
...
% features $\bx$ were generated by uniformly drawing from $[-0.5, 0.5]^d$. The OvR weights $W$ were drawn uniformly from  $[-0.5, 0.5]^{d \times m}$.
...
The conditionally dependent multi-label data was generated from a linear model whose output scores are corrupted by correlated noise. The model can written as
\begin{equation*}
\by = \assert{\boldsymbol{M} \left( \boldsymbol{A }\bx + \epsilon \right)  >  0  },
\end{equation*}
\noindent
where $\boldsymbol{A }\bx + \epsilon$ is a linear model. The noise $\epsilon$ is a $m$-dimensional vector drawn from $N(0, 0.25)$, and the $m \times m$ mixing matrix $\boldsymbol{m}$ i.i.d. from $[-1, 1]$. To make the labels dependent $\boldsymbol{M} \neq \boldsymbol{I}$. The indicator function $\assert{\ }$ applies to each element of the vector separately. The two-dimensional feature vector $\bx$ was drawn uniformly from a unit sphere, the linear coefficients $A$ uniformly from two-dimensional unit sphere. 
