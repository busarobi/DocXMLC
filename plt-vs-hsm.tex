...
probability distribution $\Pr(\bX = \bx,\bY = \by)$ (denoted later by $\Pr(\bx, \by)$) defined on $\calX \times \calY$. We denote the marginal distribution $\Pr(\bx,y_i)$ by $\Pr_i$.
...
$$
The regret quantifies the suboptimality of $\bh$ compared to the optimal classifier $\bh^*$. The goal of MLC could then be defined as finding $\bh$ with a small regret, ideally equal to zero.

Let us consider the label-wise log loss:
\begin{eqnarray*}
& & \ell_{\log}(\by, \bh(\bx))  = \sum_{j=1}^m \ell_{\log}(h_j(\bx))  \\
& & =  -\sum_{j=1}^m y_j \log(h_j(\bx)) + (1-y_j) \log(1 - h_j(\bx)) 
\end{eqnarray*}
It can be easily shown that minimization of this loss leads to estimation of the marginal probabilities, $\eta_j(\bx) = \Pr(y_j = 1 \given \bx)$. Let us derive the expected loss for a single $\bx$ (i.e., the so-called \emph{conditional risk}):
\begin{eqnarray*}
& & \mathbb{E} \ell_{\log}(\bh \given \bx) =  \sum_{j=1}^m \mathbb{E}{\ell_{\log}(y, h_j(\bx))} = \sum_{j=1}^m \loss_{\log}(h_j(\bx)) \\ 
& & =  -\sum_{j=1}^m \eta_j(\bx) \log(h_j(\bx)) + (1-\eta_j(\bx) ) \log(1 - h_j(\bx)) 
\end{eqnarray*}
Therefore, the pointwise optimal prediction for the $j$-th label is:
\begin{eqnarray*}
 h_j^*(\bx)  =  \argmin_h -\eta_j(\bx) \log(h) - (1-\eta_j(\bx) ) \log(1 - h) & &\\
=   \eta_j(\bx) & & 
\end{eqnarray*}
...
We will recall now some results that show that based on the marginal probabilities we can optimally solve the extreme classification problem for many complex performance measures, like Hamming loss, precision@k, and macro-F measure. 


In the following, we aim at estimating the marginal probabilities. This can be obtained by running the vanilla 1-vs-All approach. Since this approach is too costly in the extreme setting, we introduce an efficient algorithm based on the so-called label trees. 



Unfortunately, the Hamming loss is not well-suited to the extreme setting
in which on average only few labels are positive from a large number of all available ones.
Additionally, the large number of label forces a trade-off between time and space complexity
(which includes training and prediction time, and also the size of the model
which all grow according to the algorithm's complexity) and the accuracy for every possible label.
This makes Hamming loss not very well-suited for extreme classification problems.
Moreover, an all-zero classifier (i.e., a classifier predicting always zero) has, in fact, a very small Hamming loss.
Therefore, the ranking-based evaluation measures are often used in the extreme classification setting.
%<MW:new>
Since in many extreme multi-label learning applications it is more important to make relevant predictions
for top-$k$ positions, rather attempting to predict all the labels, which may be much more than $k$.
%</MW:new>

In this thesis, we focus on \textbf{Precision$@k$ ($P@k$)}, which reports precision at the $k$-th position
in the ranking of the most relevant labels. It is defined as:
%
\begin{equation}
%\mathrm{Precision}@k = \frac{1}{k} \sum_{\ell \in rank_k(\hy)} \by_l,
%\mathrm{Precision}@k = \frac{1}{k} \sum_{\ell \in rank_k(\hy)} \assert{y_i = 1},,
\mathrm{Precision}@k = \frac{1}{k} \sum_{i=1}^k \assert{y_{r_i(\hy)} = 1},
\label{eqn:precision-at-k}
\end{equation}
%where $rank_k(\hy)$ is a set of top-$k$ labels in some ranking of labels $\mathcal{L}$.
where $r_i(\hat\by)$ is an index of label on $i$-th position in some ranking of labels $\mathcal{L}$.
%

%\textbf{KD: Spróbuj formalnie pokazać ryzyko oraz optymalny klasyfikator dla tej miary}

%<MW:new>
The key to maximizing Precision$@k$ is to select a subset of $k$ relevant labels and rank them at the top-$k$ positions.
This can happen if the top-ranked $k$ relevant labels are not outranked by any irrelevant label.

The loss function for Precision$@k$ is $l_{P@k} = 1 - \mathrm{Precision}@k$,
then the expected loss for Precision$@k$ is given by:

\begin{eqnarray}
\loss_{P@k}(\bh,\Pr) & = & \mathbb{E}_{(\bx,\by) \sim \Pr} \left[l_{P@k}(\by,\bx) \right] =
\int 1 - \frac{1}{k} \sum_{i=1}^k \assert{y_{r_i(h(\bx))} = 1} \,d \Pr(\bx,\by) \nonumber \\
& = & \frac{1}{k} \sum_{i=1}^k \int 1 - \assert{y_{r_i(h(\bx))} = 1} \,d \Pr_i(\bx, y_{r_i(h(\bx))}) \nonumber \\
& = & \frac{1}{k} \sum_{i=1}^k \int l_{0/1}(y_{r_i(h(\bx))}, 1) \,d \Pr(\bx, y_{r_i(h(\bx))}) \nonumber \\
& = & \frac{1}{k} \sum_{i=1}^k \int l_{0/1}(y_{r_i(h(\bx))}, 1) \,d \Pr(y_{r_i(h(\bx))} | \bx)\Pr(\bx) \nonumber \\
& = & \frac{1}{k} \sum_{i=1}^k \int_\mathcal{X}
l_{0/1}(1, 1)\Pr(y_{r_i(h(\bx))} = 1 | \bx) +
l_{0/1}(0, 1)\Pr(y_{r_i(h(\bx))} = 0 | \bx) \,d \Pr(\bx) \nonumber \\
& = & \frac{1}{k} \sum_{i=1}^k \int_\mathcal{X}
l_{0/1}(0, 1)\Pr(y_{r_i(h(\bx))} = 0 | \bx) \,d \Pr(\bx) \,.
%\phantom{end_of_eqn}
%\int 1 - \frac{1}{k} \sum_{i=1}^k l_{0/1}(y_{h(\bx, i)}, 1) d \Pr(\bx,\by) \nonumber  \\
%& = & \frac{1}{k} \sum_{i=1}^k \int  l_{0/1}(y_{h(\bx, i)}, 1) d \Pr_i(\bx, y_{h(\bx, i)})\,,
\label{eqn:expected-precision-at-k}
\end{eqnarray}

%where $\eta(\bx, y_{h(\bx, 1)}) \ge \eta(\bx, y_{h(\bx, 2)}) \ge ... \ge \eta(\bx, y_{h(\bx, \size)})$

The above result show that optimal strategy for Precision$@k$ is predicting $k$ labels
with the lowest condictional probability $\Pr(y_i = 0 | \bx)$,
what is equal to predicting top-$k$ labels with the highest conditional
probability $\Pr(y_i = 1 | \bx)$.
%</MW:new>

Since the distribution of labels follows the power law,
measures like Precision$@k$ favor too much the most popular labels.
Therefore, the choice of the most suitable loss function for extreme classification setting is still an open problem.
This problem is, however, out of the scope of this thesis.




The canonical loss function for multi-label classification is Hamming loss: 

This loss is, however, not suitable for the extreme settings. We will, however, focus in the following on accurate estimation of marginal probabilities. 
...
To validate the theoretical comparison from section \ref{!} we perform a comparison of PLTs and HSM on three types of synthetic datasets: multi-label with conditionally dependent labels, multi-label with independent labels, and multi-class. 

The independent multi-label data was generated from a multi-label OvR \cite{!} model. Example features $\bx$ were generated by uniformly drawing from $[-0.5, 0.5]^d$. The OvR weights $W$ were drawn uniformly from  $[-0.5 - s, 0.5 - s]^{d \times m}$, where $s$ stands for a shift used to control the average probability of a positive label. The score $a =\bx w_i$ of $i$-th label was then transformed to a probability $p_i(a) = \frac{1}{(1 + exp(-a))}$, and the $i$-th label was drawn with the probability $p_i$.

The conditionally multi-label dependent data was generated from the PCC \cite{!} model. Example features were drawn as in the previous paragraph. The PCC model weights were drawn from  $[-0.5 - s, 0.5 - s]^{(d + m) \times m}$, also with the use of a shift $s$. The score of $i$-th label was computed based on it's features and all the previous labels $j$ as $d+j$-th features, set to -0.5 or 0.5 depending on the results of the draws with labels probabilities.

The multi-class data was generated using...

Table \ref{!} shows the empirical results of PLTs and HSM on the generated data. 
